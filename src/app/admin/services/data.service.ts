...
export class DataService {
  ...
  addProject(p: Omit<Project, 'id'>): void {
    ...
    this._projects.next([...list, { ...p, id }]);
  }

  updateProject(id: number, changes: Partial<Project>): void {
    this._projects.next(
      this._projects.getValue().map(p => p.id === id ? { ...p, ...changes } : p)
    );
  }

  deleteProject(id: number): void {
    this._projects.next(this._projects.getValue().filter(p => p.id !== id));
  }

  addSkill(s: Omit<Skill, 'id'>): void {
    const list = this._skills.getValue();
    const id   = list.length ? Math.max(...list.map(x => x.id)) + 1 : 1;
    this._skills.next([...list, { ...s, id }]);
  }

  updateSkill(id: number, changes: Partial<Skill>): void {
    this._skills.next(
      this._skills.getValue().map(s => s.id === id ? { ...s, ...changes } : s)
    );
  }

  deleteSkill(id: number): void {
    this._skills.next(this._skills.getValue().filter(s => s.id !== id));
  }
}